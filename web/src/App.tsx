...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"chat" | "skills" | "market" | "flows">("chat");
  ...
  const [marketActiveKey, setMarketActiveKey] = useState("projects");
  const [marketPage, setMarketPage] = useState<"agents" | "skills-center">("agents");
  ...
  // Auto-login (try dev-login first, works in debug mode)
  useEffect(() => {
  ...
  return (
    <div className="app-layout">
      ...
      <div className="app-body">
        {activeTab !== "market" && activeTab !== "flows" && (
          <>
        ...
        {activeTab === "market" ? (
          <Suspense fallback={<div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}><Spin /></div>}>
            <MarketSidebar
              ...
              onSelect={(key) => {
                setMarketActiveKey(key);
                if (key === "tech" || key === "efficiency" || key === "other") {
                  setMarketPage("skills-center");
                } else {
                  setMarketPage("agents");
                }
              }}
            />
